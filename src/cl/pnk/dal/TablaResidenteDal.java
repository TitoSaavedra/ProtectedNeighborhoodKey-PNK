...
public class TablaResidenteDal {
    ...
    public ObservableList<TablaResidente> obtenerTablaResidentesFiltrada(String text) {
        ...
        try {
            this.dbutils.conectar();
            String sql = "SELECT persona.RUT, persona.NOMBRE,persona.APE_PATERNO,persona.APE_MATERNO, CONCAT(direccion.PISO,' ', direccion.BLOCK,' ', direccion.NUMERO),persona.TELEFONO,persona.EMAIL FROM persona,direccion,direccion_persona WHERE direccion_persona.direccion_ID_DIRECCION = direccion.ID_DIRECCION AND direccion_persona.persona_ID_PERSONA = persona.ID_PERSONA AND persona.ID_TIPO_PERSONA='2' AND persona.ESTADO_PERSONA=1 AND persona.RUT LIKE '%" + text + "%' OR direccion_persona.direccion_ID_DIRECCION = direccion.ID_DIRECCION AND direccion_persona.persona_ID_PERSONA = persona.ID_PERSONA AND persona.ID_TIPO_PERSONA='2' AND persona.ESTADO_PERSONA=1 AND persona.NOMBRE LIKE '%" + text + "%' OR direccion_persona.direccion_ID_DIRECCION = direccion.ID_DIRECCION AND direccion_persona.persona_ID_PERSONA = persona.ID_PERSONA AND persona.ID_TIPO_PERSONA='2' AND persona.ESTADO_PERSONA=1 AND persona.APE_PATERNO LIKE '%" + text + "%' OR direccion_persona.direccion_ID_DIRECCION = direccion.ID_DIRECCION AND direccion_persona.persona_ID_PERSONA = persona.ID_PERSONA AND persona.ID_TIPO_PERSONA='2' AND persona.ESTADO_PERSONA=1 AND persona.APE_MATERNO LIKE '%" + text + "%';";
            PreparedStatement sq = this.dbutils.getConexion().prepareStatement(sql);
            ResultSet rs = sq.executeQuery();
            while (rs.next()) {
                TablaResidente tablaResidente = new TablaResidente();
